...
## 配置文件
自 1.3.0 起，在配置目录 `/data/adb/anti_safetycore` 内创建以下**无扩展名的空文件**以启用对应功能：
1. **`keep_running`**: 启用后台守护模式。模块将周期性检测 SafetyCore 与 KeyVerifier 组件状态，若发现被系统还原或版本与占位符不符，自动执行卸载与重装操作。
2. **`systemize`**: 将占位符应用提升至系统应用权限级别。在此模式下，Google Play 服务即使拥有系统签名权限，亦无法强行卸载或替换该占位符。   
      
### **兼容性提示**   
对于 KernelSU 的内核版本 ≥ 22098 或 APatch 的内核版本 ≥ 11170 的用户，如需启用系统化 (systemize) 功能，必须同时安装 **[元模块 (Metamodule)](https://kernelsu.org/guide/metamodule.html)** ，否则占位符 app 仍将作为用户应用安装。   
   
### **默认行为说明**   
模块默认以**用户应用**模式安装占位符，此配置已能满足绝大多数设备的防护需求。仅在以下场景建议额外配置：    
- 占位符 app 频繁被 Google Play 服务还原为官方版本
- 设备长期运行不重启，且担心后台组件被静默恢复
