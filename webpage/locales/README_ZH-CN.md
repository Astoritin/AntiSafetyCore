...
也就是说，如果需要防止谷歌后台阴暗爬行，只需要安装一个签名不同的占位符应用即可轻松针对。
谷歌即使因为自身的特殊性拿到了系统级权限，也无法覆盖安装或者升级/降级安装，进而就防止了谷歌在后台反复静默安装 SafetyCore 和 Key Verifier。

## 配置文件
自 1.3.0 起，在配置目录 `/data/adb/anti_safetycore` 内创建以下**无扩展名的空文件**以启用对应功能（创建文件后重启生效）：
1. **`keep_running`**: 启用后台守护模式。模块将周期性检测 SafetyCore 与 KeyVerifier 组件状态，若发现被系统还原或版本与占位符应用不符，自动执行卸载与重装操作。
2. **`systemize`**: 将占位符应用挂载为系统应用。在此模式下，Google Play 服务即使拥有系统签名权限，亦无法强行卸载或替换该占位符应用。   
...
- 占位符应用频繁被 Google Play 服务还原为官方版本
...
> 也就是说如果被谷歌成功替换回原版应用，你可以点击模块旁的操作/打开按钮以卸载当前应用，或者重新启动你的设备即可恢复为占位符应用。
